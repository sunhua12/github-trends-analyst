...
$PYTHON_CMD $FETCH_GITHUB --langs all python rust go typescript --since daily > combined_trends.json
...
if [ -z "$GEMINI_API_KEY" ]; then
    ...
    $PYTHON_CMD $AI_ANALYZER --github combined_trends.json --output $AI_FILE
fi

# 4. 生成儀表板
echo "🎨 Step 3: Generating Integrated Dashboard..."
$PYTHON_CMD $DASHBOARD_SCRIPT --github_input combined_trends.json --analysis_file $AI_FILE --template assets/dashboard_template.html --output index.html
